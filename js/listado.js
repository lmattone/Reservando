...
Listado.prototype.reservarUnHorario = function(id, horario) {
    ...
    let restaurant = this.buscarRestaurante(id);
...
Para encontrar un elemento con un determinado id, se recorre todo el arreglo de restaurantes utilizando un ciclo for. 
Esto también puede resolverse utilizando la función find().
Refactorizá la función buscarRestaurante(id) para que encuentre al 
restaurant con el id que recibe por parámetro utilizando la función find().*/
...
Listado.prototype.buscarRestaurante = function(id){
    ...
    let restaurantSelecc = this.restaurantes.find(element => element.id === id);
    if ( !!restaurantSelecc === false ) { 
        return "No se ha encontrado ningún restaurant";
    } else { return restaurantSelecc; }
...
function eliminarElem(arrays){
    let elementoUnico = arrays.filter((elem, index, self)=> index=== self.indexOf(elem));
...
Listado.prototype.obtenerUbicaciones = function() {
   ...
   /* var ciudades = [];
    ...
    return eliminarElem(ciudades).sort();
}
...
Listado.prototype.obtenerRubros = function() {
    ...
    // En este arreglo van a estar los restaurantes de un único rubro
    /*var rubrosUnicos = eliminarElem(rubros);
    return rubrosUnicos;*/
    return eliminarElem(rubros).sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    return eliminarElem(horarios).sort();
}
